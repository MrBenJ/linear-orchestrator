...
**Behavior:**
1. Validate against project mapping. Reject if `linearProjectId` is unknown.
2. Create the Linear issue via Linear API.
3. Insert `tickets` and `runs` rows. Run status = `queued`.
4. Transition Linear issue to "In Progress" (worker will own this eventually; for simplicity Phase 1 does it on enqueue).
5. Return.
...
- `Issue` state changes — when LO-tracked tickets transition (e.g., user manually reverts a ticket, or Linear's GitHub integration moves to "Done" on merge). LO records the transition; in Phase 1 no further action.
...
3. If failure: post `summary` + `notes` as a Linear comment on the ticket.
...
- Feature branch: lo/<ticket_identifier> (already checked out)
...
After /code-task finishes (PR is merged or marked merge-ready), POST to:
  ...
  Body: { "status": "success" | "failure", "prUrl": "...", "summary": "...", "notes": "..." }
...
| Worktree creation fails (dirty tree, branch exists) | Run → `failed`, reason logged, Linear comment posted |
| Agent process crashes (non-zero exit) without callback | Run → `failed`, last 50 log lines as Linear comment |
| Agent exits cleanly without callback | Run → `failed`, reason = "no callback" |
| Heartbeat lapses | Kill cascade, run → `timed_out` |
| Wall-clock exceeded | Kill cascade, run → `timed_out` |
...
2. The spawned agent runs in an isolated worktree on a fresh `lo/<identifier>` branch.
...
6. A run that completes (callback `status: success`) transitions cleanly and removes its worktree.
7. A failed run leaves the worktree intact for 7 days and posts a Linear comment.
8. Concurrency cap of 2 is enforced — three queued runs at once result in 2 running + 1 queued.
9. Integration test suite passes against the in-memory DB and echo agent.
10. A manual smoke test against a real Linear sandbox and a real claude-code run completes end-to-end.
